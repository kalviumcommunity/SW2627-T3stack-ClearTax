...
export async function getUserInvoices(userId) {
  ...
  try {
    ...
    console.warn("Database connection error in getUserInvoices, using in-memory fallback:", error.message);
    return inMemoryInvoices.filter((inv) => String(inv.userId).trim().toLowerCase() === normalizedId);
  }
}
...
export async function getInvoiceById(userId, invoiceId) {
  ...
  try {
    ...
    return result.rows[0]
      ? { ...result.rows[0], amount: Number(result.rows[0].amount) }
      : null;
  } catch (error) {
    console.warn("Database connection error in getInvoiceById, using in-memory fallback:", error.message);
    return (
      inMemoryInvoices.find(
        (inv) => String(inv.userId).trim().toLowerCase() === normalizedId && inv.id === Number(invoiceId)
      ) || null
    );
  }
}
...
export async function addUserInvoice(userId, invoice) {
  ...
  try {
    ...
    console.warn("Database connection error in addUserInvoice, using in-memory fallback:", error.message);
    const newInv = {
...
export async function updateUserInvoice(userId, invoiceId, updates) {
  ...
  try {
    ...
    console.warn("Database connection error in updateUserInvoice, using in-memory fallback:", error.message);
